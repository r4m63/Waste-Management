...
DECLARE
    ...
    v_container_exists   BOOLEAN;
    v_fraction_exists    BOOLEAN;
...
BEGIN
    ...
    SELECT EXISTS (SELECT 1
                   ...
                   WHERE id = p_container_size_id
    )
    INTO v_container_exists;

    IF NOT v_container_exists THEN
        RAISE EXCEPTION 'Container size not found';
    END IF;

    -- Валидация фракции
    SELECT EXISTS (SELECT 1
                   FROM fractions
                   WHERE id = p_fraction_id
    )
    INTO v_fraction_exists;

    IF NOT v_fraction_exists THEN
        RAISE EXCEPTION 'Fraction not found';
    END IF;
